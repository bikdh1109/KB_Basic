...
public class ProjectConfig4 {
    ...
    // 1. setMethod 이용 : 먼저 Person이 만들어지고 나서 parrot을 만들어 나중에 집어 넣는 방식
    // -> 선호하지 않음
    // 2. 생성자 이용 : 먼저 parrot을 만들어두고 Person을 만들 때, 미리 만들어 놓은 parrot 객체를 주입하여 Person을 생성
    @Bean
    ...
    public Person person(Parrot parrot){
        Person p = new Person(); // 객체 생성후, 주입하는 경우 set 메서드 이용해서 주입 가능
